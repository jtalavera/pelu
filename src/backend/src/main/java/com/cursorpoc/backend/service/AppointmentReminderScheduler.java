...
public class AppointmentReminderScheduler {
  ...
  public void sendDueReminders() {
    ...
    List<Appointment> due =
        appointmentRepository.findDueForReminder(REMINDABLE_STATUSES, windowStart, windowEnd);
  ...
  private void sendReminder(Appointment appointment) {
    ...
    // Belt-and-suspenders: findDueForReminder already filters on `reminderSentAt IS NULL`, but
    // this in-memory guard keeps "no more than one reminder per appointment" true even if a
    // future caller reuses this method against an appointment fetched a different way.
    ...
    try {
      String tenantName = appointment.getTenant().getName();
      var localStartAt = appointment.getStartAt().atZone(timeProperties.zoneId());
      String subject =
          messageSource.getMessage(
              "email.appointmentReminder.subject", new Object[] {tenantName}, REMINDER_LOCALE);
      String body =
          messageSource.getMessage(
      ...
      emailService.sendPlainTextEmail(email.trim(), subject, body);
