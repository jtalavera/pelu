...
7. **Diálogo de nuevo turno — selector de hora editable (MVP v2)** — El campo *Hora* del formulario de creación / edición de turno se presenta como un **combobox editable**: la lista desplegada incluye **todas las horas del día desde 00:00 hasta 23:45 en intervalos de 15 minutos**. El usuario puede **elegir** un valor de la lista o **escribir** una hora específica que no esté en ella; el valor escrito se acepta cuando coincide con el formato `HH:MM` (24 horas). El componente se usa de forma idéntica a la solapa *Horario* de profesionales (HU-20, criterio 6) para mantener coherencia visual y de interacción.
8. **Diálogo de nuevo turno — selector de fecha internacionalizado (MVP v2)** — El campo *Fecha* es un **componente internacionalizado**: el formato de presentación, el primer día de la semana y los nombres de meses / días siguen el **idioma activo** de la aplicación (es ⇒ `DD/MM/AAAA`, en ⇒ `MM/DD/YYYY`). El popover del calendario y todos sus textos visibles se traducen mediante i18n; el valor enviado al backend permanece como `YYYY-MM-DD` (ISO).
