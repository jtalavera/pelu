...
test.describe("HU-17 · Anular comprobante", () => {
  ...
  test("HU-17 · 3 estado anulado visible en historial", async ({ page, request }) => {
    ...
    const client = await seedClient(request, token, `E2E Void2 ${Date.now()}`);
    ...
    await page.locator("#invoice-history-text-filter").fill(client.fullName.slice(0, 12));
    await expect(
      page.locator("tbody").getByRole("row").filter({ hasText: "Voided" }),
